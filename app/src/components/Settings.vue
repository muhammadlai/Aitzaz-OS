<template>
  <div class="settings-panel p-4 h-full overflow-y-auto text-white">
    ...
    <form @submit.prevent="handleSaveAndTestSettings" v-else class="space-y-6">
      ...
      >
        <span
          >Aitzaz AI Pro v{{ appVersion }}. Owner / Creator:
...
function getToolInfo(name: string): {
  ...
  const descriptionMap: Record<string, string> = {
    get_current_datetime: 'Allows Aitzaz to get the current date and time',
    open_path:
      "Allows Aitzaz to open apps, URLs, files, and folders on the user's computer",
    manage_clipboard: "Aitzaz can read and write to the user's clipboard",
    save_memory: 'Aitzaz can store memories (long term memory)',
    delete_memory: 'Aitzaz can delete memories (long term memory)',
    recall_memories: 'Aitzaz can recall memories (long term memory)',
    list_directory:
      "Aitzaz can list the files and folders on the user's computer",
    execute_command: "Aitzaz can execute shell commands on the user's computer",
    schedule_task: 'Aitzaz can schedule tasks to run on a recurring basis',
    manage_scheduled_tasks: 'Aitzaz can manage scheduled tasks',
    ...
    search_torrents:
      'Aitzaz can search for torrents on the internet (requires Jackett)',
    add_torrent_to_qb:
      'Aitzaz can add a torrent to qBittorrent (requires qBittorrent)',
    browser_context:
      'Aitzaz can get information about the current webpage in users browser (requires browser extension)',
