...
`https://host:8765/hud/aitzaz.cer`, install profile, then enable in
...
```bash
echo "JARVIS_HUD_TOKEN=aitzaz-$(python3 -c 'import secrets;print(secrets.token_hex(3))')" >> ~/.hermes/.env
```
